...
Inboundinator =[queries.unloadPalletized(), queries.upTo1T(), queries.lz1T(), queries.lz3T(), queries.lz2T3T(),
                queries.lz1T2T(), queries.lz1000(), queries.palletPutaway()]
# Inboundinator = [queries.unloadPalletized(), queries.upTo1T(), queries.lz1T()]
# print(queries.Inboundinator())
...
pivotar = df.pivot_table(values='AMT',columns='Charge',index=['Date','Key'], fill_value=0, sort=True).reset_index()
